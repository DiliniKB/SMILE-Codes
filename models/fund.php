...
Class fund{
    ...
    function view_active_funds($cat){
    ...
    function report($data){
        ...
        for ($i=0; $i < count($data['photos']['photo']['name']); $i++) { 
            if($data['photos']['photo']['name'][$i]!="" && $data['photos']['photo']['error'][$i]== 0 )
            {
        ...
        if (!$_SESSION['error']) {
            ...
            if ($result) {
                ...
                $_SESSION['error'] = 'You can report an item only once,Contact us for more details.';
                echo $_SESSION['error'];
